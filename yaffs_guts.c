...
static int yaffs_tags_match(const struct yaffs_ext_tags *tags, int obj_id,
			    int chunk_obj)
{
if((tags->chunk_id == (u32)chunk_obj &&
...
static int yaffs_gc_block(struct yaffs_dev *dev, int block, int whole_block)
{
	...
	if (is_checkpt_block || !yaffs_still_some_chunks(dev, block)) {
		...
		if(whole_block>0)
			max_copies= dev->param.chunks_per_block;
		else 
			max_copies=5;
			     ...
			     const u8 *buffer, int n_bytes, int use_reserve)
{
	...
	if(prev_chunk_id > 0)
		new_tags.serial_number =prev_tags.serial_number + 1;
	else 
				new_tags.serial_number =1;
		     ...
		     int n_bytes, int write_through)
{
	...
	while (n > 0 && chunk_written >= 0) {
		...
		if ((start + n) < dev->data_bytes_per_chunk) {
			...
			if(n_bytes_read >(start + n)) 
				n_writeback =n_bytes_read;
			else 
				n_writeback =start + n;
		     ...
		     int discard_cache)
{
	...
	if(yaffs_update_oh(in, NULL, 0, 0, 0, NULL) >= 0) 
		return YAFFS_OK;
	else 
		return YAFFS_FAIL;
		...
		if (dev->param.is_yaffs2) {
			if (yaffs2_checkpt_restore(dev)) {
				...
				if (!init_failed
					...
					init_failed = 1;
				if (!init_failed && !yaffs2_scan_backwards(dev))/*yaffs2_scan_backwards   sorted   done  Allocating*//**//**/
					init_failed = 1;
			}
