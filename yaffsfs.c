...
int yaffsfs_CheckPath(const YCHAR *path)
{
	int n = 0;
	int divs = 0;
...
if(sharing & YAFFS_SHARE_READ)
	shareRead = 1;
else 
	shareRead = 0;

	if(sharing & YAFFS_SHARE_WRITE)
		shareWrite = 1;
	else 
		shareWrite = 0;
...
/*logMsg("get hered 1\n",1,2,3,4,5,6);*/
	...
	if(rwflags == O_RDWR || rwflags == O_RDONLY)
		readRequested = 1;
	else
		readRequested = 0;

	if(rwflags == O_RDWR || rwflags == O_WRONLY) 
		writeRequested = 1;
	else
		writeRequested = 0;
...
{
int k;
k = yaffs_open_reldir(NULL, path, oflag, mode);
	return k;
}
